...
class JWTCookieAuth(APIKeyCookie):
    ...
    def authenticate(self, request: HttpRequest, access_token: str) -> str:
        try:
            token_service = JWTTokenService()
            issued_jwt_token_service = ORMIssuedJwtTokenService()
            token_type = token_service.get_token_type_from_token(token=access_token)
            if token_type != TokenType.ACCESS:
                raise HttpError(
                    status_code=403,
                    message="Invalid token type",
                )
            if issued_jwt_token_service.check_revoked(jti=token_service.get_jti_from_token(token=access_token)):
                raise HttpError(
                    status_code=403,
                    message="Invalid token",
                )

            user_role = token_service.get_client_role_from_token(token=access_token)
            ...
            user_email = token_service.get_client_email_from_token(token=access_token)
