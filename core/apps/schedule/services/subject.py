...
class BaseSubjectService(ABC):
    ...
    @abstractmethod
    def find_any_by_title(self, title: str) -> SubjectEntity | None:
        ...
    ...
    @abstractmethod
    def soft_delete(self, subject_id: int) -> None:
        ...
...
class ORMSubjectService(BaseSubjectService):
    ...
    def get_all(self) -> list[SubjectEntity]:
        return [subject.to_entity() for subject in SubjectModel.objects.all()]

    def get_list(self, filters: SearchFilterEntity, pagination: PaginationIn) -> list[SubjectEntity]:
    ...
    def find_any_by_title(self, title: str) -> SubjectEntity | None:
        subject = SubjectModel.all_objects.filter(title=title).first()
        return subject.to_entity() if subject is not None else None
    ...
    def soft_delete(self, subject_id: int) -> None:
        is_updated = SubjectModel.objects.filter(id=subject_id).update(is_active=False)

        if not is_updated:
            raise SubjectDeleteException(id=subject_id)

    def restore(self, subject_id: int) -> None:
        SubjectModel.all_objects.filter(id=subject_id).update(is_active=True)
