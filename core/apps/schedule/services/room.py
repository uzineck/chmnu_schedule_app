...
class BaseRoomService(ABC):
    ...
    @abstractmethod
    def find_any_by_number(self, room_number: str) -> RoomEntity | None:
        ...
    ...
    @abstractmethod
    def restore(self, room_id: int) -> None:
        ...
...
class ORMRoomService(BaseRoomService):
    ...
    def get_all(self) -> list[RoomEntity]:
        return [room.to_entity() for room in RoomModel.objects.all()]
    ...
    def find_any_by_number(self, room_number: str) -> RoomEntity | None:
        room = RoomModel.all_objects.filter(number=room_number).first()
        return room.to_entity() if room is not None else None
    ...
    def soft_delete(self, room_id: int) -> None:
        is_updated = RoomModel.objects.filter(id=room_id).update(is_active=False)

        if not is_updated:
            raise RoomDeleteException(id=room_id)

    def restore(self, room_id: int) -> None:
        RoomModel.all_objects.filter(id=room_id).update(is_active=True)
