...
from core.api.v1.schedule.groups.schemas import CreateGroupSchema, GroupSchema, GroupLessonsOutSchema
...
def get_group_lessons(request: HttpRequest,
                      group_number: str,
                      filters: Query[GroupFilter]) -> ApiResponse[GroupLessonsOutSchema]:
    container = get_container()
    service: BaseGroupService = container.resolve(BaseGroupService)
    try:
        group_lessons = service.get_group_lessons(group_number=group_number,
                                                  filters=GroupFilterEntity(subgroup=filters.subgroup,
    ...
    return ApiResponse(data=GroupLessonsOutSchema(
...
def get_group_info(request: HttpRequest,
                   group_number: str) -> ApiResponse[GroupSchema]:
    container = get_container()
    service: BaseGroupService = container.resolve(BaseGroupService)
    try:
    ...
    return ApiResponse(data=GroupSchema(
        number=group.number,
...
def get_or_create_group(request: HttpRequest, schema: CreateGroupSchema) -> ApiResponse[GroupSchema]:
    ...
    service: BaseGroupService = container.resolve(BaseGroupService)
    try:
        group = service.get_or_create(group_number=schema.number,
...
@router.patch('{group_number}/add/{lesson_id}',
              response=ApiResponse[GroupSchema],
...
@router.patch('{group_number}/remove/{lesson_id}',
              response=ApiResponse[GroupSchema],
